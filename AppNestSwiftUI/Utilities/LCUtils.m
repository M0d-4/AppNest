...
+ (NSString *)liveProcessBundleIdentifier {
    ...
    NSString *bundleID = [NSString stringWithFormat:@"com.kdt.appnest.%@.LiveProcess", LCSharedUtils.teamIdentifier];
...
+ (void)launchMultitaskGuestApp:(NSString *)displayName completionHandler:(void (^)(NSNumber *pid, NSError *error))completionHandler {
    if(!self.liveProcessBundleIdentifier) {
        NSError *error = [NSError errorWithDomain:displayName code:2 userInfo:@{NSLocalizedDescriptionKey: @"LiveProcess extension not found. Please reinstall AppNest and select Keep Extensions"}];
...
+ (void)changeMainExecutableTo:(NSString *)exec error:(NSError **)error {
    NSURL *infoPath = [LCSharedUtils.appGroupPath URLByAppendingPathComponent:@"Apps/com.kdt.appnest/App.app/Info.plist"];
...
+ (NSURL *)archiveIPAWithBundleName:(NSString*)newBundleName includingExtraInfoDict:(NSDictionary *)extraInfoDict error:(NSError **)error {
    ...
    NSURL *tmpPayloadPath = [tmpPath URLByAppendingPathComponent:@"AppNest2/Payload"];
    ...
    infoDict[@"PrimaryAppNestTeamId"] = teamId;
