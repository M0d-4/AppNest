...
struct LCPath {
    ...
    public static let lcGroupDocPath = {
        ...
        if let appGroupPathUrl = LCSharedUtils.appGroupPath() {
            return appGroupPathUrl.appendingPathComponent("AppNest")
...
class SharedModel: ObservableObject {
    ...
    // 0 = current appNest is the primary one,
    // 2 = current appNest is not the primary one
    ...
    func updateMultiLCStatus() {
        if LCUtils.appUrlScheme()?.lowercased() != "appnest" {
            multiLCStatus = 2
        } else {
            multiLCStatus = 0
        }
