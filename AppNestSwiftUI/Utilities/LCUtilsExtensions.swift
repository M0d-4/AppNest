...
extension LCUtils {
    ...
    public static func askForJIT(withScript script: String? = nil, appName: String? = nil, onServerMessage: ((String) -> Void)? = nil) async -> Bool {
        // if AppNest is installed by TrollStore
        ...
        if(jitEnabler == .SideJITServer){
            ...
            if jitEnabler == .StosDebugLC {
                ...
                guard let appToLaunch else {
                    onServerMessage?("StosDebug is not installed in AppNest.")
                    return false
                }
                
                if !appToLaunch.uiIsShared {
                    onServerMessage?("StosDebug is installed in AppNest, but is not a shared app. Convert it to a shared app to continue.")
                    return false
                }
                ...
                guard let freeScheme else {
                    onServerMessage?("No free AppNest is available. Please either: \n(1)close one, \n(2)install a new one, \n(3)choose another method to enable JIT.")
                    return false
                }
                ...
                onServerMessage?("JIT acquisition will continue in another AppNest.")
            ...
            if jitEnabler == .StikJITLC {
                ...
                guard let appToLaunch else {
                    onServerMessage?("StikDebug is not installed in AppNest.")
                    return false
                }
                
                if !appToLaunch.uiIsShared {
                    onServerMessage?("StikDebug is installed in AppNest, but is not a shared app. Convert it to a shared app to continue.")
                    return false
                }
