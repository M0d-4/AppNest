...
struct LCTabView: View {
    ...
    func dispatchURL(url: URL) {
        repeat {
            ...
            switch host {
            case "appnest-launch", "install", "open-web-page", "open-url":
                sharedModel.selectedTab = .apps
            case "appnest-relaunch":
    ...
    func checkTeamId() {
        if let certificateTeamId = UserDefaults.standard.string(forKey: "LCCertificateTeamId") {
            ...
            guard let primaryLCTeamId = Bundle.main.infoDictionary?["PrimaryAppNestTeamId"] as? String else {
                print("Unable to find PrimaryAppNestTeamId")
                return
            }
