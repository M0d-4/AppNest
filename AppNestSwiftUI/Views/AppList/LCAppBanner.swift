...
struct LCAppBanner : View {
    ...
    func copyLaunchUrl() {
        if let fn = model.uiSelectedContainer?.folderName {
            UIPasteboard.general.string = "appnest://appnest-launch?bundle-name=\(appInfo.relativeBundlePath!)&container-folder-name=\(fn)"
        } else {
            UIPasteboard.general.string = "appnest://appnest-launch?bundle-name=\(appInfo.relativeBundlePath!)"
        }
    ...
    func saveIcon() async {
        ...
        let img = appInfo.generateAppNestWrappedIcon(with: style)!
