...
struct LCDataManagementView : View {
    ...
    init(appDataFolderNames: Binding<[String]>) {        
        ...
        _folderPaths = State(initialValue: [
            ...
            LCFolderPath(path: Bundle.main.bundleURL, desc: "AppNest Bundle"),
        ])
    }
    ...
    Text("lc.settings.cleanCacheConfirm".loc) 
        ...
        .alert("Reset NSUserDefaults", isPresented: $resetUserDefaultsAlert.show) {
            ...
            Text("This will completely reset NSUserDefaults to fix issue due to corruption (supports both legacy and new storage systems). AppNest will restart with clean preferences. Your app data in Files/AppNest is preserved.")
        }
    ...
    func resetUserDefaults() async {
        ...
        do {
            ...
            for i in 0..<8 {
                let suiteName = "com.kdt.appnest.userDefaultsStorage.\(i)"
            ...
            if fm.fileExists(atPath: preferencesPath.path) {
                ...
                for file in contents {
                    ...
                    // pattern matching for ALL AppNest preference files
                    let shouldDelete = fileName.contains(bundleId) || 
                                    fileName.contains("AppNest") ||
                                    fileName.contains("appnest") ||  // Case insensitive
                                    fileName.contains("userDefaultsStorage") ||
                                    fileName.hasPrefix("com.kdt.appnest") ||
                                    fileName.matches(pattern: "com\\.kdt\\.appnest.*\\.plist") ||
                                    fileName.matches(pattern: ".*userDefaultsStorage.*\\.plist") ||
                                    fileName.matches(pattern: ".*appnest.*\\.plist")
            ...
            let sharedDomains = [
                "com.kdt.appnest.shared",
                "group.com.kdt.appnest",
                bundleId + ".shared"
            ]
            ...
            successInfo = "NSUserDefaults reset complete (\(detectedSystems) systems). Cleared \(totalCleared) preference entries, \(deletedFiles) files (\(totalDeletedSize / 1024)KB), and \(clearedSharedDomains) shared domains. AppNest will restart immediately. Your app data in Files/AppNest is preserved."
            successShow = true
