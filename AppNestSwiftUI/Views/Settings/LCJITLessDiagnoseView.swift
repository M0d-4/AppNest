    ...
    func onAppear() {
        ...
        if let entitlementTeamId {
            ...
            if let keyChainAccessGroups = entitlementDict["keychain-access-groups"] as? Array<String> {
                var notFound = true
                if keyChainAccessGroups.contains("\(entitlementTeamId).com.kdt.appnest.shared") {
                    notFound = false
                    for i in 1..<SharedModel.keychainAccessGroupCount {
                        if !keyChainAccessGroups.contains("\(entitlementTeamId).com.kdt.appnest.shared.\(i)") {
                            notFound = true
                            continue
                        }
                    }
...
struct LCJITLessDiagnoseView : View {
    ...
    func getHelp() {
        UIApplication.shared.open(URL(string: "https://appnest.github.io/docs/faq/jit-less-mode-setup")!)
    }
