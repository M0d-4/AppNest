...
- (UIImage *)generateAppNestWrappedIconWithStyle:(GeneratedIconStyle)style {
...
- (NSDictionary *)generateWebClipConfigWithContainerId:(NSString*)containerId iconStyle:(GeneratedIconStyle)style{
    NSString* appClipUrl;
    if(containerId) {
        appClipUrl = [NSString stringWithFormat:@"appnest://appnest-launch?bundle-name=%@&container-folder-name=%@", self.bundlePath.lastPathComponent, containerId];
    } else {
        appClipUrl = [NSString stringWithFormat:@"appnest://appnest-launch?bundle-name=%@", self.bundlePath.lastPathComponent];
    }
    
    UIImage* icon = [self generateAppNestWrappedIconWithStyle:style];
    ...
    NSDictionary *payload = @{
        ...
        @"PayloadDescription": [NSString stringWithFormat:@"Web Clip for launching %@ (%@) in AppNest", self.displayName, self.bundlePath.lastPathComponent],
        ...
        @"PayloadVersion": @(1),
        @"Precomposed": @NO,
        @"toPayloadOrganization": @"AppNest",
        @"URL": appClipUrl
    };
    return @{
        @"ConsentText": @{
            @"default": [NSString stringWithFormat:@"This profile installs a web clip which opens %@ (%@) in AppNest", self.displayName, self.bundlePath.lastPathComponent]
        },
        @"PayloadContent": @[payload],
        @"PayloadDescription": payload[@"PayloadDescription"],
        @"PayloadDisplayName": self.displayName,
        @"PayloadIdentifier": self.bundleIdentifier,
        @"PayloadOrganization": @"AppNest",
...
- (void)save {
    ...
    if(!_autoSaveDisabled) {
        ...
        guestAppInfo[@"hideAppNest"] = @(self.hideAppNest);
...
- (void)patchExecAndSignIfNeedWithCompletionHandler:(void(^)(bool success, NSString* errorInfo))completetionHandler progressHandler:(void(^)(NSProgress* progress))progressHandler forceSign:(BOOL)forceSign {
    ...
    if (needPatch || forceSign) {
        ...
        NSString *backupPath = [NSString stringWithFormat:@"%@/%@_AppNestPatchBackUp", appPath, _infoPlist[@"CFBundleExecutable"]];
        NSError *err;
...
- (bool)hideAppNest {
    if(_info[@"hideAppNest"] != nil) {
        return [_info[@"hideAppNest"] boolValue];
    } else {
        return YES; // default to YES
    }
}
- (void)setHideAppNest:(bool)hideAppNest {
    _info[@"hideAppNest"] = [NSNumber numberWithBool:hideAppNest];
    [self save];
}
