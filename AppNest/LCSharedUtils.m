    ...
    dispatch_once(&once, ^{
        ...
        for (NSString *group in possibleAppGroups) {
            ...
            if ([NSFileManager.defaultManager fileExistsAtPath:bundlePath.path]) {
                // This will fail if AppNest is installed in both stores, but it should never be the case
                appGroupID = group;
                return;
            }
        }
...
+ (BOOL)launchToGuestApp {
    ...
    if (!urlScheme) {
        tries = 2;
        urlScheme = [NSString stringWithFormat:@"%@://appnest-relaunch", lcAppUrlScheme];
    }
...
+ (BOOL)launchToGuestAppWithURL:(NSURL *)url {
    ...
    if(![components.host isEqualToString:@"appnest-launch"]) return NO;
    ...
    if(launchBundleId) {
        ...
        // Attempt to restart AppNest with the selected guest app
...
+ (NSURL*)containerLockPath {
    ...
    dispatch_once(&once, ^{
        infoPath = [[LCSharedUtils appGroupPath] URLByAppendingPathComponent:@"AppNest/containerLock.plist"];
    });
    return infoPath;
}
...
// lc can be something like appnest or appnest2.liveprocess, such that one LC can jump to another LC hosting the multitask app when user presses run while it's running
...
+ (void)moveSharedAppFolderBack {
    ...
    NSURL *docPathUrl = [fm URLsForDirectory:NSDocumentDirectory inDomains:NSUserDomainMask]
        .lastObject;
    NSURL *appGroupFolder = [[LCSharedUtils appGroupPath] URLByAppendingPathComponent:@"AppNest"];
    ...
    if(!appGroupFolder && sharedDataFoldersToMove.count > 0) {
        [lcUserDefaults setObject:@"AppNest was unable to move the data of shared app back because AppNest cannot access app group. Please check JITLess diagnose page in AppNest settings for more information." forKey:@"error"];
        return;
    }
...
+ (NSBundle*)findBundleWithBundleId:(NSString*)bundleId isSharedAppOut:(bool*)isSharedAppOut {
    ...
    if (!appBundle) {
        appGroupFolder = [[LCSharedUtils appGroupPath] URLByAppendingPathComponent:@"AppNest"];
...
+ (void)dumpPreferenceToPath:(NSString*)plistLocationTo dataUUID:(NSString*)dataUUID {
    ...
    for(int i = 0; i < 8; ++i) {
        NSUserDefaults* cur = [[NSUserDefaults alloc] initWithSuiteName:[NSString stringWithFormat:@"com.kdt.appnest.userDefaultsStorage.%d", i]];
...
+ (NSArray<NSString*>*)lcUnorderedUrlSchemes {
    NSArray<NSString *> *defaultSchemes = @[@"appnest", @"appnest2", @"appnest3"];
