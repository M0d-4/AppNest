...
@implementation NSUserDefaults(AppNest)
...
static NSString* invokeAppMain(NSString *selectedApp, NSString *selectedContainer, int argc, char *argv[]) {
    ...
    if (!LCSharedUtils.certificatePassword && !isSideStore) {
#if !TARGET_OS_SIMULATOR
        if(@available(iOS 26.0 ,*))  {
            return @"JITLess mode is required since iOS 26. Please set it up in settings. \nPlease go to AppNest settings -> tap \"Import Certificate from SideStore\" / \"Import Certificate\"";
        }
#endif
        ...
        if (!checkJITEnabled()) {
            appError = @"JIT was not enabled. If you want to use AppNest without JIT, setup JITLess mode in settings.";
            return appError;
        }
    }
    ...
    if(!guestAppInfo) {
        ...
        appGroupFolder = [appGroupPath URLByAppendingPathComponent:@"AppNest"];
    ...
    *_CFGetProgname() = NSProcessInfo.processInfo.processName.UTF8String;
    ...
    DyldHooksInit([guestAppInfo[@"hideAppNest"] boolValue], hookDlopen, [guestAppInfo[@"spoofSDKVersion"] unsignedIntValue]);
    ...
    if([guestAppInfo[@"dontInjectTweakLoader"] boolValue] && ![guestAppInfo[@"dontLoadTweakLoader"] boolValue]) {
        tweakLoaderLoaded = true;
        if([guestAppInfo[@"hideAppNest"] boolValue]) {
...
int AppNestMain(int argc, char *argv[]) {
    ...
    if(lastLaunchDataUUID) {
        ...
        if([lastLaunchType isEqualToString:@"Shared"]) {
            preferencesTo = [LCSharedUtils.appGroupPath.path stringByAppendingPathComponent:[NSString stringWithFormat:@"AppNest/Data/Application/%@/Library/Preferences", lastLaunchDataUUID]];
        } else {
            NSString *docPath = [NSString stringWithFormat:@"%s/Documents", getenv("LC_HOME_PATH")];
    ...
    // however if the running lc is liveprocess and current lc is appnest1 we just continue
    if(selectedApp && runningLC) {
        ...
        dispatch_after(delay, dispatch_get_main_queue(), ^{
            ...
            if(selectedContainer) {
                urlStr = [NSString stringWithFormat:@"%@://appnest-launch?bundle-name=%@&container-folder-name=%@", runningLC, selectedAppBackUp, selectedContainer];
            } else {
                urlStr = [NSString stringWithFormat:@"%@://appnest-launch?bundle-name=%@", runningLC, selectedAppBackUp];
            }
    ...
    void *AppNestSwiftUIHandle = dlopen("@executable_path/Frameworks/AppNestSwiftUI.framework/AppNestSwiftUI", RTLD_LAZY);
    NSCAssert(AppNestSwiftUIHandle, @"%s", dlerror());
    ...
    if ([lcUserDefaults boolForKey:@"LCLoadTweaksToSelf"]) {
        ...
        if (isSharedBundle) {
            NSURL *appGroupPath = [NSFileManager.defaultManager containerURLForSecurityApplicationGroupIdentifier:[LCSharedUtils appGroupID]];
            tweakFolder = [appGroupPath.path stringByAppendingPathComponent:@"AppNest/Tweaks"];
        } else {
    ...
    int (*AppNestSwiftUIMain)(void) = dlsym(AppNestSwiftUIHandle, "main");
    return AppNestSwiftUIMain();
