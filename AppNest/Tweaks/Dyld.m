...
static NSString *appNestBundleId = nil;
...
//             strstr(lowerImageName, "appnestshared"));   // AppNestShared
// }
...
static bool shouldHideLibrary(const char* imageName) {
    ...
    return (strstr(lowerImageName, "substrate") ||
            ...
            strstr(lowerImageName, "appnestshared"));
}
...
static void ensureAppMainIndexIsSet(void) {
    ...
    // Find the guest app executable (not AppNest)
    ...
    for(int i = 0; i < imageCount; ++i) {
        ...
        if(currentImageHeader && currentImageHeader->filetype == MH_EXECUTE &&
           imageName && i != lcImageIndex && !strstr(imageName, "AppNest.app")) {
...
// Helper for AppNest special case handling
// static bool isAppNestImage(uint32_t imageIndex, const char* imageName) {
//     return imageIndex == lcImageIndex || (imageName && strstr(imageName, "AppNest"));
// }

// static uint32_t handleAppNestReplacement(uint32_t imageIndex) {
...
uint32_t hook_dyld_image_count(void) {
    ...
    // Count visible (non-hidden) images INCLUDING AppNest (which will be replaced)
...
// }
const struct mach_header* hook_dyld_get_image_header(uint32_t image_index) {
    // ALWAYS handle AppNest replacement first (at virtual index level)
...
void hideAppNestImageCallback(const struct mach_header* header, intptr_t vmaddr_slide) {
    Dl_info info;
    dladdr(header, &info);
...
static BOOL shouldUseAppNestBundleId(void) {
    // Get the call stack to determine context
    NSArray *callStack = [NSThread callStackSymbols];

    // Check for system framework calls that need AppNest's bundle ID
...
static NSString *LCResolvedBundleIDForCurrentContext(void) {
    ...
    if (useSelectiveBundleIdSpoofing) {
        BOOL useLCBundleID = shouldUseAppNestBundleId();
        NSString *bundleIDToExpose = useLCBundleID ? appNestBundleId : originalGuestBundleId;
...
static void configureSelectiveBundleIdSpoofing(NSDictionary *guestAppInfo, BOOL hideAppNest) {
    ...
    appNestBundleId = entitlementBundleID ?: NSUserDefaults.lcMainBundle.bundleIdentifier;
    if (appNestBundleId.length == 0) {
        appNestBundleId = NSBundle.mainBundle.bundleIdentifier;
    }

    BOOL forceHostBundle = [guestAppInfo[@"doUseLCBundleId"] boolValue];
    useSelectiveBundleIdSpoofing = hideAppNest || forceHostBundle;
...
static void lc_applyGuestContextConfiguration(NSDictionary *guestAppInfo, bool hideAppNest) {
    lc_configureNetworkSpoofing(guestAppInfo);
    bypassSSLPinning = [guestAppInfo[@"bypassSSLPinning"] boolValue];
    configureSelectiveBundleIdSpoofing(guestAppInfo, hideAppNest);
...
static void lc_installDyldCoreHooks(bool hideAppNest) {
    // Hook dlsym to solve RTLD_MAIN_ONLY, hook other functions to hide AppNest itself.
...
// MARK: Init
void DyldHooksInit(bool hideAppNest, bool hookDlopen, uint32_t spoofSDKVersion) {
    ...
    lc_applyGuestContextConfiguration(guestAppInfo, hideAppNest);
    lc_resolveDyldRuntimeContext();
    lc_installDyldCoreHooks(hideAppNest);
    ...
    appExecutableFileTypeOverwritten = !hideAppNest;
...
kern_return_t catch_mach_exception_raise_state( mach_port_t exception_port, exception_type_t exception, const mach_exception_data_t code, mach_msg_type_number_t codeCnt, int *flavor, const thread_state_t old_state, mach_msg_type_number_t old_stateCnt, thread_state_t new_state, mach_msg_type_number_t *new_stateCnt) {
    ...
    if(pc == (uint64_t)machOChainedFixupsValidLinkedit) {
        *new = *old;
        *new_stateCnt = old_stateCnt;
        static char emptyValidLinkeditBuffer[100] = "Create an issue on AppNest GitHub if you see this.";
