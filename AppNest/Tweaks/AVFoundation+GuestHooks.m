...
@interface NSUserDefaults(AppNestPrivate)
...
static void setupAudioSpoofingResources(void) {
    if (!audioSpoofingQueue) {
        audioSpoofingQueue = dispatch_queue_create("com.appnest.audiospoofing", DISPATCH_QUEUE_SERIAL);
    }
...
static void initializePhotoCacheQueue(void) {
    ...
    dispatch_once(&onceToken, ^{
        photoCacheQueue = dispatch_queue_create("com.appnest.photocache", DISPATCH_QUEUE_SERIAL);
    });
}
...
@implementation AVCaptureDevice(AppNestSpoof)
...
@implementation AVCaptureDeviceInput(AppNestSpoof)
...
@implementation AVCaptureSession(AppNestSpoof)
...
@implementation AVCaptureVideoDataOutput(AppNestSpoof)
...
@implementation AVCaptureAudioDataOutput(AppNestSpoof)
...
@implementation AVCaptureDataOutputSynchronizer(AppNestSpoof)
...
@implementation AVAssetWriter(AppNestSpoof)
...
@implementation NSFileManager(AppNestSpoof)
...
@implementation AVCapturePhotoOutput(AppNestSpoof)
...
@implementation AVCaptureStillImageOutput(AppNestSpoof)
...
@implementation AVCaptureMovieFileOutput(AppNestSpoof)
            ...
            @try {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (delegate && [delegate respondsToSelector:@selector(captureOutput:didFinishRecordingToOutputFileAtURL:fromConnections:error:)]) {
                        NSError *spoofError = [NSError errorWithDomain:@"AppNestSpoof" 
                                                                 code:-1 
...
@implementation AVCaptureVideoPreviewLayer(AppNestSpoof)
...
void AVFoundationGuestHooksInit(void) {
    @try {
        ...
        if (!videoProcessingQueue) {
            videoProcessingQueue = dispatch_queue_create("com.appnest.videoprocessingqueue", DISPATCH_QUEUE_SERIAL);
        }
