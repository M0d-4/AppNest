...
int AppNestMainC(int argc, char *argv[]) {
...
loadlc:
    lcShared = dlopen("@executable_path/Frameworks/AppNestShared.framework/AppNestShared", RTLD_LAZY|RTLD_GLOBAL);
    lcMain = dlsym(lcShared, "AppNestMain");
...
int main(int argc, char *argv[]) {
    ...
    if(lcShared == NULL) {
        __attribute__((musttail)) return AppNestMainC(argc, argv);
    }
